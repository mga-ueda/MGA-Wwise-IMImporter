...
| Auto Volume | More Options 内・上段右（Loudness Normalize の隣）。`Auto Volume`（既定オフ）／調整先 `Make-Up Gain`（既定）または `Voice Volume`（縦一列）。Normalize で変化した音量の逆を Music Playlist へ書き戻す。Normalize オフ時は無効。`[Project.*] AutoVolumeEnabled`／`AutoVolumeTarget` に自動保存 |
